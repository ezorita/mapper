...
)
{
   ...
   uint64_t wrdptr = (wrdnum + wrdnum/OCC_MARK_INTERVAL + 1)*NUM_BASES;
   uint64_t mrkptr = (((wrdnum + OCC_MARK_INTERVAL/2)/OCC_MARK_INTERVAL)*(OCC_MARK_INTERVAL+1))*NUM_BASES;
...
)
{
   ...
   uint64_t wrdnum = ptr/OCC_WORD_SIZE;
   uint64_t wrdptr = (wrdnum + wrdnum/OCC_MARK_INTERVAL + 1)*NUM_BASES + nt;
   uint64_t mrkptr = (((wrdnum + OCC_MARK_INTERVAL/2)/OCC_MARK_INTERVAL)*(OCC_MARK_INTERVAL+1))*NUM_BASES + nt;
   uint64_t bit    = ptr%OCC_WORD_SIZE;
   ...
   if (wrdptr > mrkptr) {
      ...
      // Returm sum.
      return occ[markptr] + offset;
   } else {
      // Sum partial word.
      if (bit < OCC_WORD_SIZE - 1)
         offset += __builtin_popcountl(occ[wrdptr] << (bit+1));
      // Sum bit offsets.
      for (uint64_t i = wrdptr + NUM_BASES; i < markptr; i+= NUM_BASES)
         offset += __builtin_popcountl(occ[i]);
      // Return subtraction.
      return occ[markptr] - offset;
   }
...
{
   uint64_t occ_sp[NUM_BASES];
   uint64_t occ_ep[NUM_BASES];
   uint64_t fp[NUM_BASES];
   uint64_t rp[NUM_BASES];
   int64_t sz[NUM_BASES];
   compute_occ(pos.fp - 1, index->occ, occ_sp);
   compute_occ(pos.fp + pos.sz - 1, index->occ, occ_ep);
   for (int j = 0; j < NUM_BASES; j++) {
...
)
{
   ...
   fmdpos_t newpos = backward_ext(nt, index, (fmdpos_t) {pos.rp, pos.fp, pos.sz});
